...
const htmlParts = [
  ...
  '    function createEntry(type) { var listDiv = document.getElementById("file-list"); var div = document.createElement("div"); div.className = "file-item new-entry-row"; div.innerHTML = "<div class=\'file-name-container\'>" + (type==="folder"?"📁":"📄") + " <input type=\'text\' class=\'rename-input\'></div>"; listDiv.insertBefore(div, listDiv.firstChild); var input = div.querySelector("input"); input.focus(); input.onblur = function() { var name = input.value.trim(); if(!name) { div.remove(); return; } var newKey = currentPrefix + name + (type==="folder"?"/":""); apiFetch("/api/put/" + encodeURIComponent(newKey), { method: "POST" }).then(() => reloadList()); }; input.onkeydown = function(e) { if(e.key==="Enter") input.blur(); }; }',
  ...
  '        if(editor.disabled) { alert("此文件无法编辑保存"); return; }',
