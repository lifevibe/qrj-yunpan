 ...
 * * 🔧 Fixed: 解决浏览器缓存导致保存后看不到修改的问题 (v=timestamp)
 * * 🔧 Fixed: 增强自动保存逻辑 (切换文件/文件夹/返回时自动触发)
 * * 💅 UI: 按钮交互反馈 (复制成功打钩、下载加载动画)
 * * 💅 UI: 全新设计的警示风格删除确认弹窗
 */
...
const htmlParts = [
  ...
  '        <div class="logo-area"><span>🗯𝑳𝒊𝒇𝒆𝑽𝒊𝒃𝒆</span></div>',
  '        <div class="header-actions">',
  '           <button class="glass-btn" onclick="triggerFileUpload()" title="上傳文件">⬆️上傳文件</button>',
  ...
  '           <button class="glass-btn btn-icon" onclick="reloadList()" title="刷新">🔄刷新</button>',
  ...
  '        </div>',
  '      </div>',
  '      <textarea id="editor" placeholder="// 👉🏾 選擇一個文件..."></textarea>',
  ...
  '              <div class="audio-art" id="audio-art-icon">🎶</div>',
  ...
  '                  <button class="play-pause-btn" id="audio-play-btn" onclick="toggleAudio()">⏯</button>',
  '              </div>',
  '          </div>',
  ...
  '              <span class="info-icon-large" id="np-icon">📄</span>',
  '              <span class="info-filename" id="np-filename">Filename.ext</span>',
  '              <span class="info-meta" id="np-meta">0 KB | Unknown Type</span>',
  '              <span class="warning-text" id="np-warning">❕ 此文件不支持在線預覽或包含二進制內容</span>',
  '              <div style="display:flex; flex-direction:column; gap:10px; width:100%;">',
  '                    <button class="primary" id="np-download-btn" style="width:100%;">⚡️ 下載文件</button>',
  '                    <button class="glass-btn" id="np-force-edit-btn" style="width:100%; font-size:12px; color:#aaa;">🖊強行編輯 (文本)</button>',
  '              </div>',
  '          </div>',
  '      </div>',
  '',
  ...
  '      <div class="menu-item" onclick="execMenu(\'open\')">✅ 打開</div>',
  '      <div class="menu-item" onclick="execMenu(\'share\')">🌐 在綫瀏覽🅞分享</div>',
  '      <div class="menu-separator"></div>',
  '      <div class="menu-item" onclick="execMenu(\'rename\')">🗄 重命名</div>',
  '      <div class="menu-item" onclick="execMenu(\'cut\')">🗄 剪切</div>',
  '      <div class="menu-item" onclick="execMenu(\'copy\')">🗄 複製</div>',
  '      <div class="menu-item" id="menu-paste" onclick="execMenu(\'paste\')">🗄 粘貼</div>',
  '      <div class="menu-separator"></div>',
  '      <div class="menu-item" onclick="execMenu(\'move\')">🗄 移動</div>',
  '      <div class="menu-item" onclick="execMenu(\'copy_link\')">🗄 複製鏈接</div>',
  '      <div class="menu-item" onclick="execMenu(\'download\')">🗄 下載</div>',
  '      <div class="menu-separator"></div>',
  '      <div class="menu-item" onclick="execMenu(\'info\')">ℹ️ 屬性</div>',
  '      <div class="menu-item" onclick="execMenu(\'delete\')" style="color:var(--danger-color)">🗑 刪除</div>',
  '  </div>',
  '',
  ...
  '           <span class="login-title">登錄</span>',
  '           <div class="login-input-group"><input type="text" id="login-user" class="login-input" placeholder="輸入用戶名" autocomplete="username"></div>',
  '           <div class="login-input-group"><input type="password" id="login-pass" class="login-input" placeholder="输入密码" autocomplete="current-password"></div>',
  '           <button class="primary login-btn" onclick="performLogin()">登錄</button>',
  '       </div>',
  '    </div>',
  '  </div>',
  '',
  ...
  '    <div class="modal-box">',
  '       <div class="modal-header"><span id="move-title">移動文件</span><span class="modal-close" onclick="closeModal(\'move-modal\')">✕</span></div>',
  '       <div class="modal-body">',
  '           <p style="color:#aaa; margin-bottom:5px; font-size:12px;">選擇目標文件夹:</p>',
  '           <select id="move-folder-select" class="folder-select" onchange="updateMoveInput()"></select>',
  '           <p style="color:#aaa; margin-bottom:5px; font-size:12px;">目標完整路徑 (可編輯):</p>',
  ...
  '           <button class="primary" onclick="submitMove()">確認移動</button>',
  '       </div>',
  '    </div>',
  '  </div>',
  '',
  ...
  '    <div class="modal-box">',
  '       <div class="modal-header"><span>⬇️ 雲下載 (遠程上傳)</span><span class="modal-close" onclick="closeModal(\'cloud-dl-modal\')">✕</span></div>',
  '       <div class="modal-body">',
  '           <p style="color:#aaa; margin-bottom:5px; font-size:12px;">輸入文件 URL:</p>',
  '           <input type="text" id="cloud-dl-url" class="full-width-input" placeholder="https://example.com/file.zip" spellcheck="false">',
  '           <p style="color:#aaa; margin-bottom:5px; font-size:12px; margin-top:10px;">(文件將直接保存當前目錄)</p>',
  '       </div>',
  '       <div class="modal-footer">',
  '           <button class="glass-btn" onclick="closeModal(\'cloud-dl-modal\')">取消</button>',
  '           <button class="primary" onclick="submitCloudDL()">開始下載</button>',
  '       </div>',
  '    </div>',
  '  </div>',
  '',
  ...
  '           <button class="glass-btn" onclick="closeModal(\'confirm-modal\')" style="padding: 8px 24px; font-size:14px;">取消</button>',
  '           <button id="confirm-btn-ok" class="primary danger-btn" style="padding: 8px 30px; font-size:14px;">確認刪除</button>',
  '       </div>',
  '    </div>',
  '  </div>',
  '',
  ...
  '    }',
  '    audioEl.addEventListener("play", () => { audioPlayBtn.innerHTML = "⏸"; audioArt.classList.add("playing"); });',
  '    audioEl.addEventListener("pause", () => { audioPlayBtn.innerHTML = "▶"; audioArt.classList.remove("playing"); });',
  ...
  '        } else if (action === "download") { if(ctxIsFolder) showToast("暂不支持文件夹下载", "error"); else downloadFile(null, ctxTargetKey);',
  ...
  '            // 按钮部分使用了 this 传参',
  '            var html = "<div class=\'file-name-container\'><span class=\'file-icon\'>" + icon + "</span> <span class=\'file-name\' title=\'" + f.key + "\'>" + displayName + "</span></div><div class=\'actions\'><button class=\'icon-btn\' onclick=\'viewFile(\\"" + f.key + "\\")\'>🌐</button><button class=\'icon-btn info-btn\' onclick=\'showFileInfo(\\"" + f.key + "\\")\'>ℹ</button><button class=\'icon-btn rename-btn\' onclick=\'triggerRename(this, \\"" + f.key + "\\", false)\'>✎</button><button class=\'icon-btn move-btn\' onclick=\'triggerMove(\\"" + f.key + "\\", false)\'>➜</button><button class=\'icon-btn copy-btn\' onclick=\'copyLink(this, \\"" + keySafe + "\\")\'>🔗</button><button class=\'icon-btn\' onclick=\'downloadFile(this, \\"" + keySafe + "\\")\'>⬇</button><button class=\'icon-btn del-btn\' onclick=\'deleteFile(\\"" + f.key + "\\")\'>×</button></div>";',
  '            div.innerHTML = html;',
  ...
  '        showConfirmDialog(',
  '            "永久删除文件?", ',
  '            "您确定要删除 <span class=\'confirm-file-highlight\'>" + key.split(\'/\').pop() + "</span> 吗？<br>此操作无法撤销。", ',
  '            () => {',
  '                showToast("正在删除...", "info");',
  ...
  '        showConfirmDialog(',
  '            "删除文件夹?", ',
  '            "您确定要删除文件夹 <span class=\'confirm-file-highlight\'>" + key + "</span> 及其包含的所有内容吗？<br>此操作<b>无法恢复</b>。", ',
  '            () => {',
  '                showToast("正在删除文件夹...", "info");',
  ...
  '        event.stopPropagation(); document.getElementById("info-modal").style.display = "flex"; document.getElementById("modal-content").innerHTML = "🌀加載中..."; ',
  ...
  '            html += renderRow("📁 路径", info.key);',
  '            html += renderRow("📦 大小", formatSize(info.size));',
  '            html += renderRow("🏷️ 类型", info.contentType || "Unknown");',
  '            html += renderRow("📅 修改时间", new Date(info.uploaded).toLocaleString());',
  '            html += "</div>";',
  ...
  '        var html = "<div style=\'text-align:center; margin-bottom:20px;\'><div style=\'font-size:48px; margin-bottom:10px;\'>📂</div><div style=\'font-size:16px; font-weight:bold; word-break:break-all; color:#fff;\'>" + prefix + "</div></div>";',
  '        html += "<div class=\'info-list\'>" + renderRow("类型", "Folder / Directory") + "</div>";',
  '        document.getElementById("modal-content").innerHTML = html; ',
  '    }',
  '    /* --- 核心: 新建文件/文件夹输入框自动提交 (修复Blur失效) --- */',
  ...
  '    }',
  '',
  '    /* --- 核心: 打开文件 (增强混合加载版 + TS Support + 大文件/二进制卡片视图) --- */',
  ...
  '                  }',
  '              } else {',
  '                  /* 🛡️ 新增: 智能判断 (大文件 或 二进制文件) -> 统一显示卡片视图 */',
  '                  ',
  '                  /* 定义常见不支持直接预览的二进制/压缩包/文档格式 */',
  ...
  '                        ',
  '                        /* 动态修改提示语: 是太大，还是格式不支持? */',
  '                        var reason = info.size > 20 * 1024 * 1024 ? "文件过大" : "不支持在线预览的格式";',
  '                        document.getElementById("np-warning").innerText = "⚠️ " + reason + " (建议下载)";',
  '                        ',
  '                        // 绑定下载事件',
  ...
  '        ',
  '        var newName = filenameInput.value.trim(); ',
  '        if (!newName) return showToast("文件名不能为空", "error");',
  '',
  '        var newKey = currentPrefix + newName;',
  '        ',
  ...
  '                        if (!confirm("重命名失败 (" + t + ")。是否保存为新文件?")) return; ',
  '                    } ',
  '                } catch(e) { ',
  '                    showToast("重命名错误: " + e, "error"); return; ',
  '                }',
  '            }',
  '        }',
  '',
    ...
    async fetch(request, env) {
      ...
      if (url.pathname.startsWith('/api/get/')) {
        ...
        try {
          ...
          // Fix: 缩短缓存时间为 60s，避免编辑器保存后立刻读取到旧文件
