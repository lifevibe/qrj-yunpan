...
const htmlParts = [
  ...
  '',
  '    /* --- 媒体预览容器 --- */',
  '    .preview-container { display: none; flex: 1; justify-content: center; align-items: center; overflow: hidden; background: rgba(0,0,0,0.1); padding: 20px; }',
  ...
  '    #audio-preview audio { width: 80%; outline: none; }',
  ...
  '      <div id="audio-preview" class="preview-container"><audio controls></audio></div>',
  '    </div>',
  '  </div>',
  '',
  ...
  '             } else if (isAudio) {',
  '                 hideAll(); audioContainer.style.display = "flex"; editorEl.disabled = true;',
  '                 var blob = await (await apiFetch("/api/get/" + encodeURIComponent(key))).blob();',
  '                 currentMediaUrl = URL.createObjectURL(blob);',
